...
def parse_arguments():
    ...
    parser.add_argument("--logging_dir", type=str, default='', help="Logging dir dur")
...
def main():
    ...
    dataset = load_dataset("sepidmnorozy/Bulgarian_sentiment")
    ...
    model.add_adapter("bg_sa")

    # a classification head for our target task
    model.add_classification_head("bg_sa", num_labels=2)

    # specify which adapter to train
    model.train_adapter(["bg_sa"])
    print(model.adapter_summary())
    model.config.prediction_heads['bg_sa']['dropout_prob'] = 0.5
    print(model)
