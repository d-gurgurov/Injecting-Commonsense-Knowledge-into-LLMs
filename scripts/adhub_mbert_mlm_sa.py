...
def parse_arguments():
    ...
    parser.add_argument("--logging_dir", type=str, default="./training_output", help="Logging directory for training results")
...
def main():
    ...
    lang_adapter_config = AdapterConfig.load(args.adapter_config, reduction_factor=16)
    print(lang_adapter_config)
    model.load_adapter(args.adapter_dir, config=lang_adapter_config, load_as="ms_wiki", with_head=False)

    # a new down-stream task adapter
    model.add_adapter("ms_sa")

    # a classification head for our target task
    model.add_classification_head("ms_sa", num_labels=2)

    # specify which adapter to train
    model.train_adapter(["ms_sa"])

    model.config.prediction_heads['ms_sa']['dropout_prob'] = 0.5
    print(model.config.prediction_heads['ms_sa']['dropout_prob'])

    # unfreeze and activate stack setup
    model.active_adapters = Stack("ms_wiki", "ms_sa")

    model.config.prediction_heads['ms_sa']['dropout_prob'] = 0.5
    print("Dropout value:", model.config.prediction_heads['ms_sa']['dropout_prob'])
